...
import { IActualizarEvento } from "../ports/IActualizarEvento"
import { ActualizarEventoRequest, ActualizarEventoSchema } from "../dto/ActualizarEventoRequest"
...
export class ActualizarEvento implements IActualizarEvento {
  constructor(private readonly eventoRepository: IEventoRepository) {}

  async execute(request: ActualizarEventoRequest): Promise<Result<EventoResponse>> {
    try {
      const validParams = ActualizarEventoSchema.parse(request)
      ...
      await Promise.all([
        this.eventoRepository.eliminarActividades(evento.id),
        this.eventoRepository.eliminarMedia(evento.id),
        this.eventoRepository.eliminarParticipantes(evento.id),
        this.eventoRepository.eliminarPatrocinadores(evento.id),
        this.eventoRepository.eliminarIntereses(evento.id),
      ])
